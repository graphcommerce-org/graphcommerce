...
function resolveRecursivePackageJson(packageJsonFilename, dependencyStructure, force = false) {
  ...
  dependencyStructure[packageJson.name] = {
    dirName: node_path_1.default.dirname(
      node_path_1.default.relative(process.cwd(), packageJsonFilename),
    ),
    dependencies,
  }
  dependencies.forEach((dependency) => {
    resolveRecursivePackageJson(
      require.resolve(node_path_1.default.join(dependency, 'package.json')),
      dependencyStructure,
    )
  })
...
function resolveDependenciesSync(root = process.cwd()) {
  ...
  const dependencyStructure = resolveRecursivePackageJson(
    node_path_1.default.join(root, 'package.json'),
    {},
    true,
  )
