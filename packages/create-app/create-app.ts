...
export async function createApp({
  ...
  if (example) {
    ...
    // console.log()
  } else if (parentFolder === 'packages' || parentFolder === 'examples') {
    console.log(
      isNotEmpty
    ...
    updateJson(
      ...
      },
      {
        sideEffects: false,
        engines: { node: '12.x' },
        prettier: '@reachdigital/prettier-config',
        browserlist: ['extends @reachdigital/browserslist-config'],
        eslint: { extends: '@reachdigital/eslint-config' },
      },
    )

    const dependencies = [
      //todo: remove version constraint when actual versions are being published
      '@reachdigital/typescript-config@^0.1.0',
      '@reachdigital/eslint-config@^0.1.0',
      '@reachdigital/browserslist-config@^0.1.0',
    ]
    ...
    updateJson(
      ...
      { exclude: ['node_modules'], include: ['**/*.ts', '**/*.tsx'] },
      { extends: '@reachdigital/typescript-config/config.json' },
    )
