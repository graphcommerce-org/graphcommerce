...
export function useGoogleTagManagerListItemHandler(item: ProductToGtagItemFragment) {
...
export function GoogleTagManagerItemList<
  P extends ProductToGtagItemFragment = ProductToGtagItemFragment,
>(props: UseGtagViewItemListProps<P> & { children: React.ReactNode }) {
  ...
  useEffect(
    () => globalThis.dataLayer.push({ event: 'view_item_list', ecommerce: eventData }),
    [eventData],
  )
