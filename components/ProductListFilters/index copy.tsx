...
import { Chip, List, ListProps, ListSubheader } from '@material-ui/core'
...
export default function ProductListFilters(props: ProductFiltersProps) {
  const { aggregations, params, filterTypeMap, ...listProps } = props

  return (
    <List subheader={<li />} {...listProps}>
      {aggregations.map((aggregation) => {
        if (aggregation.attribute_code === 'category_id') return null
        return (
          <li key={aggregation.attribute_code}>
            <ul>
              <ListSubheader>{aggregation.label}</ListSubheader>
